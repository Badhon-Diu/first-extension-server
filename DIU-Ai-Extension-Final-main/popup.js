...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function initTabs() {
    ...
    async function processVoiceAudio(base64Audio, assessment) {
      ...
      if (!base64Audio) {
        showStatus(voiceStatus, 'Recording was empty — please record again.', 'error');
    ...
    function addFiles(files) {
      // Only types the server's Image API actually accepts (PDF is NOT supported
      // by either the server's upload filter or the external Image API).
      ...
      if (skipped > 0) {
        showStatus(uploadStatus, skipped + ' file(s) skipped — only PNG, JPG, WEBP or GIF are supported (PDF is not supported).', 'error');
      }
      updateFileList();
    }
    ...
    processBtn.addEventListener('click', async function() {
      ...
      // The server's upload filter accepts a maximum of 10 images per request.
      if (uploadedFiles.length > 10) {
        showStatus(uploadStatus, `Maximum 10 images per batch. Remove ${uploadedFiles.length - 10} image(s).`, 'error');
        return;
      }
      
      // OPTIONAL BUT RECOMMENDED: Prevent popup crash from huge files
      const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB limit per file
      for (let file of uploadedFiles) {
        if (file.size > MAX_FILE_SIZE) {
          showStatus(uploadStatus, `File ${file.name} is too large. Max 10MB.`, 'error');
          return;
        }
      }
      ...
      try {
        ...
        // FIX: Correctly extract the array whether it's a raw array or an object with a "results" property
        const resultItems = Array.isArray(apiData) ? apiData : (apiData.results || []);
        aiGeneratedResults = matchResultsToStudents(resultItems, studentData);
